...
**日期**: 2026-01-22
**位置**: TMC4361A 编程文档扩展（页 161-200）

### 本次完成

- 读取 TMC4361 Datasheet 第 161-200 页内容
- 将内容整理并写入 `documents/TMC4361A_Programming_Guide.md`
- 新增章节：
  - 第 26 章：闭环操作详解
    - 闭环基本参数（CL_OFFSET、CL_BETA、CL_TOLERANCE、CL_DELTA_P）
    - 闭环校准与启用（自动校准、使用已知偏移值）
    - 闭环追赶速度限制（PI 调节器参数）
    - 闭环速度模式
    - 闭环电流缩放（CL_IMIN、CL_IMAX、过渡控制）
    - 反电动势补偿（γ 校正）
    - 编码器速度滤波
    - PID 控制参数详解
- 文档版本更新为 1.4
- 提交：`2afc277 更新 TMC4361A 编程手册至 v1.3`
...
TMC4361A 编程指南现已涵盖数据手册第 1-200 页核心内容，包含 26 个章节和 2 个附录。
第 161-200 页主要包含闭环操作详细配置和完整寄存器列表（技术规格部分）。
