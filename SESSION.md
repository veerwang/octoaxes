...
**日期**: 2026-02-26（续）
**分支**: develop
**位置**: 命令层 Bug 修复 + 二进制协议迁移 + 速度/加速度功能实现

### 本次完成

#### 1. HOME_OR_ZERO 轴值编码修复（b3878b6）
...
#### 2. enable/disable 轴迁移到二进制协议（5cd4359）
...
1. **硬件验证 homing 修复**（X/Y 归位是否正确，不再互换）
2. **硬件验证速度/加速度设置**（Z 轴调速测试）
3. **硬件验证 TTL 端口 + DAC**（D1-D5 通断 + DAC80508 模拟强度）
4. **修正 W 轴 config.h 配置**（LEFT_SW → RGHT_SW + 极性修正）
5. **去掉 StepAxis homing debug 打印**（确认稳定后）
6. **响应机制**：固件 `sendResponse()` 尚未在命令处理后调用，上位机依赖定时刷新
