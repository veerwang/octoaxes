...
void ISR_strobeTimer()
{
    ...
    for (int i = 0; i < NUM_TRIGGER_CHANNELS; i++) {
        ...
        // 2026-07-20 审计 F-1 修复（与 octoaxesplus 同步）：原短曝光（≤30ms）路径在 ISR
        // 内用 delayMicroseconds 阻塞（最长 30ms/帧，冻结主循环：位置广播/命令 ack/
        // 电机状态机轮询全部延迟）。统一为异步两步式（原长曝光路径），开/关灯时刻受
        // 100µs 定时器粒度限制：照明窗口精度 ±100µs（10ms 曝光 ≈ ±1%，显微成像无感）；
        // on_time < 100µs 时实际最短点亮约一个定时器周期。
        if (!strobe_on[i]) {
