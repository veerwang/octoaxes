...
void Axis::startMovement() {
  ...
  // doesn't wait too long. velMM uses _config.maxVelocityMM (already updated by host speed changes).
...
void Axis::configureStagePID(bool flip_direction, uint16_t transitions_per_rev) {
  ...
  if (strcmp(_axisName, "W") == 0 || strcmp(_axisName, "W2") == 0) {
    ...
    // Conversion: 10 microsteps = 0.28° motor = ±0.10° turret (÷2.75 gear ratio, 12800 µstep/motor rev).
    // 20 = ±0.20° turret. After tightening, back off if PID jitters/whines. (the W filter wheel stays at 20)
    target_tolerance = 10;
    pid_tolerance = 10;
    pid_iclip = 4096;
