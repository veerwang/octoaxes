...
namespace AxisConfigs {
    ...
    // Z轴配置
    // ───────────────────────────────────────────────────────────────────
    // 新旧 Z 变体编译开关（与 software/octoaxes/constants.py Z_AXIS_VARIANT 配对，需手动保持一致）
    //   新 Z (MOONS' LE143S-W0601)：坐标翻转(firmware 正=物理左、朝 home 走 XACTUAL 增大)。
    //     【2026-06-09 项目把正负限位传感器物理对调】：负/home 端现接 TMC4361A STOPR 引脚（原接
    //     STOPL）。该物理对调恰好抵消坐标翻转 → INVERT_STOP_DIRECTION 由 true 改 false（Z_FLIPPED
    //     =false）：home 逼近=正向内部速度，STOPR(默认)拦正向→home 硬停；上限位移到 STOPL 拦负向
    //     →过冲硬停。STATUS：home 接 STOPR 引脚、不翻转→直接读作 STOPR 位 → homingSwitch 仍 RGHT_SW。
    //     极性 1；homing 仍以负限位为回零点(homing_direct=1)。（对调前 Z_FLIPPED=true，见 git 历史）
    //   旧 Z：标准约定，home 在右限位、极性 0、不翻转、chip 硬停启用。
    // ⚠️ 限位极性/翻转值同步自 octoaxesplus（含 06-09 传感器对调）。octoaxes 主线板是另一连接器/接线，
    //    新 Z 限位行为【尚未在 octoaxes 板上机实测】（octoaxesplus 板对调已读数佐证，octoaxes 板未测）
    //    ——须用 software/common/tests/z_limit_monitor.py 在 octoaxes 板验证 STOPL/STOPR 极性与硬停方向
    //    后再定稿（参考 Turret LEFT/RIGHT 反面教材）。
    ...
    const Axis::AxisConfig Z_AXIS = {
        .clockFrequency = SystemConfig::TMC4361_CLOCK_FREQUENCY,
        .homingSwitch = Z_HOMING_SWITCH,      // 变体开关：新旧 Z 都 RGHT_SW（新 Z 2026-06-09 传感器对调后 home 接 STOPR 引脚、不翻转，直接读作 STOPR 位）
