...
// zero all 8 DAC channel outputs (verified during ttl_test bring-up)
void dac_zero_all();

// read a DAC80508 register using the two-frame SPI protocol (addr range 0x00..0x18)
uint16_t read_DAC8050x_reg(uint8_t addr);

// main-loop hook: after entering loop, do a one-time CONFIG/GAIN/zero sync
// to work around a hardware bug where the first setup-time SPI transaction is occasionally dropped (verified during ttl_test bring-up)
