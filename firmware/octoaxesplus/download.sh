#!/usr/bin/env bash
# Octoaxes Plus（squid++ 双相机）主控固件烧录脚本
#   ./download.sh                交互选择
#   ./download.sh safe           启用激光联锁（pin 38 需接联锁信号，常规出厂版本）
#   ./download.sh nointerlock    禁用激光联锁（无联锁工位用，否则 D1-D8 荧光通道点不亮）
set -e
cd "$(dirname "$0")"
...
if [ -z "$choice" ]; then
    echo "请选择烧录版本："
    echo "  1) 启用联锁 (safe)        - 需要 pin 38 接激光联锁信号才能开 D1-D8 TTL 端口"
