  ...
  for (uint8_t i = 0; i < axisCount; i++) {
    if (axes[i] != nullptr) {
      ...
      if (!success) {
        ...
        DEBUG_PRINTLN(axisName);
        // 2026-06-02 aligned with octoaxes: a begin() failure means the TMC4361A SPI did not respond
        // (board not plugged in / chip damaged / broken wiring). Delete this Axis instance and set the slot to nullptr,
        // so later findAxisByName returns nullptr, and every handler's if (axis) guard automatically
        // turns commands into a silent no-op (the response packet reports any_moving=false and COMPLETED immediately, so the host's
        // wait_till_operation_is_completed wakes up at once, without a 5s stall). This avoids later SPI operations
        // hitting a dead chip, wasting the bus, and false-positive status dragging down the whole system.
        delete axes[i];
        axes[i] = nullptr;
        allSuccess = false;
      } else {
