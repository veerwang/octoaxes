...
class WAxisHomingTest:
    ...
    def connect(self, port=None):
        ...
        try:
            ...
            self.port = port
            time.sleep(0.5)  # wait for the serial port to stabilize
    ...
    def send_binary_command(self, cmd, axis_index, params=None):
        ...
        try:
            ...
            # format: [prefix(1), CMD(1), axis index(1), params(3), CRC(1)]
            data = bytearray(8)
            data[0] = 0x02  # binary-command prefix
            data[1] = cmd
            data[2] = axis_index
    ...
    def start_w_axis_homing(self):
        ...
        # method 1: use a string command
        print("[INFO] 发送 W 轴 homing 命令（字符串方式）...")
        self.send_string_command("W:HOMING")

        # or use a binary command (alternative)
        # print("[INFO] sending the W-axis homing command (binary method)...")
    ...
    def monitor_homing_progress(self, timeout=30.0):
        ...
        while time.time() - start_time < timeout:
            # query the status once per second
