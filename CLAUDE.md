...
**当前进度**: Z 编码器启用 + XYZ 速度基线 + Homing 调试打印清理 + `Axis::moveRelativeMicrosteps` 静默 reject 修复 + 旧 Squid X 卡死 root cause + 协议层下降沿立即发包优化 + **Y homing 异响实测确定方案（256 微步 + 30 mm/s）+ 调试基础设施（S:SET_HOMING_VEL 命令 + diag_y_homing_noise.py 交互扫描脚本）**；响应包保持 24 字节与旧 Squid 兼容
**下一步**: 提交 Y homing 修复 + 工具脚本；TMC2240 SG4 调优（如再切回 TMC2240）；W 轴 60ms 优化；继续框架效率优化（STATUS.TARGET_REACHED 判完 / Target Pipeline / 多轴并行 home）
