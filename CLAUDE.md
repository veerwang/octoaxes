...
**最后更新**: 2026-03-16
**当前进度**: 固件重构完成 + TMC2240 驱动支持已添加
**下一步**: TMC2240 硬件验证 + 功能测试和优化

### TMC2240 驱动支持 (2026-03-16)

新增 TMC4361A + TMC2240 驱动芯片支持，与现有 TMC2660 并存：

- **配置方式**: `config.h` 中每轴 `.driverType = DRIVER_TMC2660` 或 `DRIVER_TMC2240`
- **通信差异**: TMC2660 用 20-bit Cover (format 0x0A)，TMC2240 用 40-bit Cover (format 0x09)
- **向后兼容**: 所有现有轴默认 DRIVER_TMC2660，行为完全不变
- **新增文件**: `tmc/ic/TMC2240/` 目录（3 个文件）
- **待验证**: 需要实际 TMC2240 硬件进行通信和电流参数验证
