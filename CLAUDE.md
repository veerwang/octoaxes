...
**当前进度**: Z 编码器启用 + XYZ 速度基线（TMC2240 首次归档，对比 TMC2660 差异 < 1%）+ Homing 调试打印清理 + `Axis::moveRelativeMicrosteps` 静默 reject bug 修复（仿老 Squid 覆盖语义）；响应包保持 24 字节与旧 Squid 兼容
**下一步**: 启用 TMC2240 StealthChop2 解决 Y homing 异响；W 轴 60ms 优化；Cover40 调试代码清理
