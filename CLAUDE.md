...
**最后更新**: 2026-05-12
**当前硬件**: X/Y/Z/W 全部 TMC2240（之前所有基线/SESSION 描述中的 X/Y/Z=TMC2660 是更早一个硬件单元）
**当前进度**: Z 编码器启用 + XYZ 速度基线（TMC2240 首次归档）+ Homing 调试打印清理 + `Axis::moveRelativeMicrosteps` 静默 reject 修复 + 旧 Squid X 卡死 root cause（TMC2240 SG4 误触发 ACTIVE_STALL_F latch）修复（按芯片分离启用）+ **协议层下降沿立即发包优化**（短距离命令 -5ms）；响应包保持 24 字节与旧 Squid 兼容
**下一步**: TMC2240 SG4 调优 + chip-level latch 恢复修复；启用 StealthChop2 解 Y homing 异响；W 轴 60ms 优化；继续框架效率优化（STATUS.TARGET_REACHED 判完 / Target Pipeline / 多轴并行 home）
