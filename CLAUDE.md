...
**当前进度**: Z 编码器启用 + XYZ 速度基线（TMC2240 首次归档）+ Homing 调试打印清理 + `Axis::moveRelativeMicrosteps` 静默 reject 修复 + 旧 Squid X 卡死 root cause（TMC2240 SG4 误触发 ACTIVE_STALL_F latch）修复（按芯片分离启用）；响应包保持 24 字节与旧 Squid 兼容
**下一步**: TMC2240 SG4 调优 + chip-level latch 恢复修复；启用 StealthChop2 解 Y homing 异响；W 轴 60ms 优化
