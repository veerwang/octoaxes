...
### 2026-07-09 注释英文化分支十三轮同步 main（chore/translate-comments-to-english）
...
- [x] **验证**：十三轮均无冲突标记残留、merge-touched 文件注释零中文、固件 pio run SUCCESS（一/二/三/五/六/七/八/十/十一/十二/十三轮）/ py_compile + profile 加载/verify_profiles OK（四/八/九/十轮）。已 push `origin/chore/translate-comments-to-english`（与 main 完全同步）。
- [ ] （可选）开 PR 合回 main。
...
- [x] **review 遗留 ③ 清理** `3131440` — 删死变量 `strobe_output_level[]`（全仓确认仅剩定义/extern/init 写入，无任何读取；实际角色由 `strobe_on[]` 承担）。三 env 编译 SUCCESS。
- [x] **ISR 共享变量补 volatile**（两固件 trigger.h/cpp + illumination.h/cpp，未提交）— 三提交 review 观察点：ISR 与主循环共享变量此前全非 volatile（靠 noInterrupts 内存屏障兜着，上游 Squid 给 illumination_source/illumination_is_on 标了）。按「ISR 共享即标」补齐：illumination_source/illumination_is_on + trigger 六个数组（control_strobe/strobe_on/strobe_active_source/strobe_delay_us/illumination_on_time_us/timestamp_trigger_rising_edge）；trigger_output_level 仅主循环使用刻意不标（注释说明）。三 env 编译 SUCCESS、两固件 diff 逐行一致。
