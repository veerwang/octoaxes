...
- [ ] **Y homing 异响 + 慢（用老 Squid software + octoaxes firmware 组合）** (2026-05-11 部分修复，**未完全解决**) — 老 Squid software + 老 Squid firmware 正常；老 Squid software + octoaxes firmware Y homing 出现异响 + 速度慢。**已识别两条差异**：① **电流公式语义不匹配**：老 Squid software 发的字段是 RMS，老 Squid firmware 公式 `(RMS_mA/1000) × R_sense / 0.2298 × 31` 当 RMS 处理；octoaxes firmware `calculateCurrentScale` 当 PEAK 处理 → 实际 RMS 低 ~30%。② **Y homing 速度公式不联动**：老 Squid `HOMING_VELOCITY_Y(0.8) × MAX_VELOCITY_Y(30) = 24mm/s`；octoaxes `HOMING_VELOCITY_Y_MM = 10mm/s` 绝对值，不跟 vmax 走。**D2 修复**（已实施，未提交）：`MotorControl.cpp::calculateCurrentScale` 公式改 RMS 解读 `CS = RMS_A × R × 32 × √2 / 0.310 - 1`，影响 X/Y/Z 三 TMC2660 轴电流 +46~47%（W TMC2240 不受影响）。修复后 CS: X/Y=31(clamp满)、Z=30，实际 RMS X/Y≈0.997A、Z≈0.494A，目标值 1A/1A/0.5A。**实测结果**：异响**仍有**（虽然估计减弱）；速度慢未改。**下一步可疑点**：(a) 微步差异（老 Squid HCS 配 32 微步、octoaxes 配 256，虽然 GUI 下发 32 时 firmware 已切到 32 但 chopper / 内部 CFG 可能未同步）；(b) StealthChop/Chopper 配置 (TOFF/HSTRT/HEND/TBL/SGT)；(c) homing 加速度参数；(d) 方案 E 未做（homing 速度联动 vmax）。
